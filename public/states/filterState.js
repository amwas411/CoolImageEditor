...
export class FilterState extends BaseState {
    ...
    render() {
        ...
        let imageData = this.coolImageEditor.mainCtx.getImageData(0, 0, this.coolImageEditor.mainCanvas.width, this.coolImageEditor.mainCanvas.height);
        if (+this.coolImageEditor.filterConfig["sharpen"]) {
            this.applySharpen(this.coolImageEditor.mainCtx, imageData, +this.coolImageEditor.filterConfig["sharpen"]);
        }
        if (+this.coolImageEditor.filterConfig["vignette"]) {
            this.applyVignette(this.coolImageEditor.mainCtx, imageData, +this.coolImageEditor.filterConfig["vignette"]);
        }
        if (+this.coolImageEditor.filterConfig["grain"]) {
            this.applyGrain(this.coolImageEditor.mainCtx, imageData, +this.coolImageEditor.filterConfig["grain"]);
        }
        if (+this.coolImageEditor.filterConfig["warmth"]) {
            this.applyWarmth(this.coolImageEditor.mainCtx, imageData, +this.coolImageEditor.filterConfig["warmth"]);
        }
    ...
    applyVignette(ctx, imageData, value) {
    ...
    applyWarmth(ctx, imageData, value) {
    ...
    applyGrain(ctx, imageData, value) {
        const data = imageData.data;
        for (let i = 0; i < data.length; i += 4) {
    ...
    applySharpen(ctx, imageData, value) {
        let kernel = [
            0, 0, 0,
            0, 1, 0,
            0, 0, 0
                ...
                for (let ky = 0; ky < kernelSide; ++ky) {
                    for (let kx = 0; kx < kernelSide; ++kx) {
                        ...
                        if (cy >= imageData.height)
                            cy = imageData.height - 1;
                        if (cx < 0)
                            cx = 0;
                        if (cx >= imageData.width)
                            cx = imageData.width - 1;
